...
class ThemeExtractor:
    ...
    def extract_fallback(event_desc: str, interests: str) -> List[str]:
        ...
        # Simple extraction of compound words / frequent words
        # e.g., 'smart cities', 'sustainable development' if adjacent
        # But for simplicity, we can do a frequency count and grab top unique keywords/phrases
        seen = set()
        themes = []
        
        # Check for multi-word interests or event terms first
        # E.g. Split inputs by comma to capture phrases directly entered by user
        for phrase in re.split(r'[,;.]', f"{event_desc}, {interests}"):
            ...
            if phrase_clean and len(phrase_clean) > 3 and phrase_clean not in STOPWORDS:
                # Limit phrase length to 4 words
